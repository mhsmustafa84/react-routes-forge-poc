...
const FEATURES = [
  ...
  { title: "Next.js integration", path: PATHS.NEXT, label: "Next.js" },
  { title: "Relative & Locale URLs", path: PATHS.DEBUG, label: "New in v1.6" },
  ...
  return (
    <div>
      <section className="hero">
        <p className="hero-kicker">Type-safe routing for React · v1.6.0</p>
        ...
        <p className="hero-sub">
          Every route lives in a single source of truth. Path builders, relative
          URLs, locale prefixes, nested query params, and active matching are
          inferred with compile-time type safety.
        </p>
      ...
      <section className="feature-grid" aria-label="Demo areas">
        {FEATURES.map((f) => (
          <button key={f.title} className="feature-card" onClick={() => navigate(f.path)}>
            <h3>{f.title}</h3>
            <p>
              <code>{f.path}</code>
            </p>
